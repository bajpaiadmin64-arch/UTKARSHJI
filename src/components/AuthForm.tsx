...
export default function AuthForm({ onSuccess, embedMode = false }: AuthFormProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setErrorMsg('Google Sign-In is not enabled on this Firebase project yet. Please enable it in the console.');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setErrorMsg('Email/Password Sign-In is not enabled on this Firebase project. To enable it: Go to Firebase Console > Authentication > Sign-in method, click "Add new provider", select "Email/Password", enable it, and save.');
      ...
      <form onSubmit={handleSubmit} className="space-y-4" id="auth-inputs-form">
        ...
        <div className="relative my-4" id="auth-divider">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-slate-900/85 px-2 text-slate-500 font-mono">Or continue with</span>
          </div>
        </div>
        ...
        <div>
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center space-x-2 py-3 border border-slate-800 rounded-xl text-xs font-semibold uppercase tracking-wider text-slate-350 hover:bg-slate-850/50 transition-all active:scale-95"
            id="auth-google-btn"
          >
            <svg className="w-4 h-4 text-slate-350" viewBox="0 0 24 24">
              <path
                fill="currentColor"
